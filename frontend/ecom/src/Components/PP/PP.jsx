        ...
        <section className="py-12 sm:py-16">
          <div className="container mx-auto px-4">
              ...
              <ol role="list" className="flex items-center">
                ...
                <li className="text-left">
                  <div className="flex items-center">
                    <span className="mx-2 text-gray-400">/</span>
                    <div className="-m-1">
                      <a href="#" className="rounded-md p-1 text-sm font-medium text-gray-600 focus:text-gray-900 focus:shadow hover:text-gray-800" aria-current="page">{products.name}</a>
                    </div>
                  </div>
                </li>
              </ol>
              ...
              <div className="lg:col-span-3 lg:row-end-1">
                <div className="lg:flex lg:items-start">
                  <div className="lg:order-2 lg:ml-5">
                    <div className="h-92 w-92 bg-red-500 overflow-hidden rounded-lg">
                      <img className="h-full w-full object-cover w" src={"http://localhost:4000/" + products.images[index]} alt="" />
                    </div>
                  </div>
            ...
            <button type="button" class="flex-0 aspect-square mb-3 h-20 overflow-hidden rounded-lg border-2 border-gray-900 text-center">
                <img class="h-full w-full object-cover" src={"http://localhost:4000/" + products.images[0]} onClick={()=>{setindex(0)}} alt="" />
              </button>
              <button type="button" class="flex-0 aspect-square mb-3 h-20 overflow-hidden rounded-lg border-2 border-gray-900 text-center">
                <img class="h-full w-full object-cover" src={"http://localhost:4000/" + products.images[1]} onClick={()=>{setindex(1)}} alt="" />
              </button>
              <button type="button" class="flex-0 aspect-square mb-3 h-20 overflow-hidden rounded-lg border-2 border-transparent text-center">
                <img class="h-full w-full object-cover" src={"http://localhost:4000/" + products.images[2]}  onClick={()=>{setindex(2)}} alt="" />
              </button>
              <button type="button" class="flex-0 aspect-square mb-3 h-20 overflow-hidden rounded-lg border-2 border-transparent text-center">
                <img class="h-full w-full object-cover" src={"http://localhost:4000/" + products.images[3]} onClick={()=>{setindex(3)}} alt="" />
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="lg:col-span-2 lg:row-span-2 lg:row-end-2">
                <h1 className="sm: text-2xl font-bold text-gray-900 sm:text-3xl">{products.name}</h1>

                <div className="mt-5 flex items-center">
                  <div className="flex items-center">
                    {[...Array(5)].map((_, i) => (
                      <svg key={i} className="block h-4 w-4 align-middle text-yellow-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
            ...
            <div className="lg:col-span-3">
              ...
              <div className='grid grid-cols-1 md:grid-cols-4 gap-4 w-[80vw] mt-4'>
                {[...Array(5)].map((_, i) => (
                  <Review key={i} />
                ))}
              </div>
